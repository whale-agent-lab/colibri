...
static uint64_t expert_record_bytes(const ColiSafetensorsIndex *index) {
    ...
    for (size_t i = 0; i < sizeof(parts) / sizeof(parts[0]); i++) {
        ...
        if (!tensor || tensor->nbytes < 0 ||
                UINT64_MAX - total < (uint64_t)tensor->nbytes) return 0;
...
static void v4_serve_one(ColiV4Engine *engine, ColiV4Session *session,
    ...
    int result = coli_v4_session_generate(
        ...
        &(ColiV4SessionGenerateOptions){
            ...
            .stop_at_sentence = 0,
            .no_dspark = engine->runtime.dspark_model_dir == NULL,
        },
...
static int v4_serve_main(void) {
    ...
    const char *draft_model_dir = getenv("COLI_V4_DSPARK_MODEL");
    const char *no_dspark_env = getenv("COLI_V4_NO_DSPARK");
    int no_dspark = no_dspark_env && *no_dspark_env &&
                     strcmp(no_dspark_env, "0") &&
                     strcmp(no_dspark_env, "false");
    ...
    ColiV4EngineOpenOptions open_options = {
        ...
        .dspark_model_dir = draft_model_dir,
