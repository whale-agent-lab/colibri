...
int coli_v4_dspark_layer_load(ColiV4Engine *engine,
    ...
    /* A session runner owns its safetensors index and is rebuilt for each
     * request. Resident stage bytes are owned by the engine, whose copied
     * dspark_model_dir is immutable, so pointer identity across runners is
     * neither stable nor a valid model-identity check. */
